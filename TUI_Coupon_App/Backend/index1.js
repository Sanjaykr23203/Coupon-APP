...
app.delete('/data/:ticketId', async (request, response) => {
    try {
        const ticketId = request.params.ticketId;
        const coupon = await Coupon.findOne({ Ticket_ID: ticketId });
        if (!coupon) {
            return response.status(404).json({ message: 'Coupon not found' });
        }
        await Coupon.deleteOne({ Ticket_ID: ticketId });
...
app.get('/data/:ticket_Id', async (request, response) => {
    try {
        const { ticket_Id } = request.params;
        // Use the 'ticket_Id' parameter to query the database
        const coupon = await Coupon.findOne({ Ticket_ID : ticket_Id });
        if (!coupon) {
            return response.status(404).json({ message: 'Coupon not found' });
        }
        return response.status(200).json({
